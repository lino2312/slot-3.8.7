...
import MoneyComingData from "db://assets/games/MoneyComing/script/MoneyComingData";

import { Node, _decorator } from 'cc';
const { ccclass, property } = _decorator;

@ccclass('MoneyComingItemAni')
export default class MoneyComingItemAni extends BaseComponent {
    ...
    playAni (index: number = 0, loop: boolean = false, cbComplete: Function = null, cbEvent: Function = null) {
        ...
        let script = this.ndAni.getComponent(MySpine);
        script.setTimeScale(MoneyComingData.SPINE_TIMESCALE);
