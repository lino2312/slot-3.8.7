...
class BezierCurve {
	...
	private _resetData(): void {
		...
		/** 首尾相等 */
		const equalsB = Vec3.strictEquals(this._pointV3S[0], this._pointV3S[this._pointV3S.length - 1]);
		/** 总距离 */
		let sumDistanceN = 0;
		/** 临时变量 */
		let tempV3: Vec3;
	...
	private _curve(
		...
		point3V3_: Vec3,
		point4V3_: Vec3
	): (((xN: number) => Vec3) | ((xN: number) => number))[] {
		// 基本样条线插值算法
		// 弹性
		const sN = 0.5;
		...
		const b3V3 = pointV3_.clone().multiplyScalar(-sN).add(point3V3_.clone().multiplyScalar(sN));
		const b4V3 = point2V3_;

		// 函数曲线
		function fx(xN: number): Vec3 {
			return bV3
				.clone()
	...
	point(posN_: number): Vec3 | null {
		...
		const resultV3 = new Vec3();
		const indexN = this._pointV3S.length - 1;
	...
	/** 匀速点 */
	uniformPoint(posN_: number): Vec3 | null {
		let posN = posN_;
		...
		// 平均距离
		const averDistN = posN * this._distanceNS[this._pointV3S.length - 2];
		let indexN = 0;
		let beyondN = 0;
		let percentN = 0;
		...
		for (let kN = 0; kN < this._pointV3S.length - 1; kN++) {
			if (averDistN < this._distanceNS[kN]) {
				const preDis = kN === 0 ? 0 : this._distanceNS[kN - 1];
		...
		for (let kN = 0; kN < 6; kN++) {
			const actualLenN = this._gaussLegendre(this._funcFSS[indexN][1] as any, 0, aN);

			bN = aN - (actualLenN - beyondN) / this._funcFSS[indexN][1](aN);
