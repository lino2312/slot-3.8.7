import { BlockInputEvents, Component, Node, Prefab, UIOpacity, UITransform, Vec3, _decorator, assetManager, find, instantiate, screen, tween, view } from 'cc';
import { App } from 'db://assets/scripts/App';
import { PopUpAnimType } from 'db://assets/scripts/component/PopupComponent';
import { PopUpManager } from "db://assets/scripts/manager/PopUpManager";
const { ccclass, property } = _decorator;

@ccclass('SlotGameHelpBase')
export class SlotGameHelpBase2 extends Component {
    ...
    onLoad() {
        ...
        this._itemWidth = uiTrans.getComponent(UITransform).contentSize.width;
        ...
        this.ensureUIOpacity(lay_bg).getComponent(UIOpacity).opacity = 0;

        this.node.y = - screen.windowSize.height;
    }
    ...
    ShowUIAction() {
        ...
        this.node.setPosition(this.node.position.x, -screen.windowSize.height, this.node.position.z);
        layBgOpacity.getComponent(UIOpacity).opacity = 0;
        ...
        tween(this.node)
            ...
            .call(() => {
                tween(layBgOpacity.getComponent(UIOpacity))
                    .to(0.3, { opacity: 150 })
                    .start();
            })
            .start();
    ...
    OnClickBackGame() {
        ...
        const winSize = view.getVisibleSize();
        tween(this.node)
            .to(0.5, { position: new Vec3(this.node.position.x, -screen.windowSize.height, this.node.position.z) })
            .call(() => {
